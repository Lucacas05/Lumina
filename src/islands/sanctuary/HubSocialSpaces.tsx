            ...
            <div className={["absolute inset-0", isAnonymous ? "blur-[4px]" : ""].join(" ")}>
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-6 md:max-w-[48%]">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest text-primary">{sharedLibrary.status}</span>
                </div>
                <h3 className="text-2xl font-headline font-black uppercase tracking-tighter text-white">{sharedLibrary.title}</h3>
                <p className="mt-2 max-w-lg text-sm font-body text-on-surface-variant">{sharedLibrary.description}</p>
              </div>
            </div>
          ...
          <a href={isAnonymous ? undefined : garden.href} className="relative flex min-h-[18rem] flex-1 overflow-hidden bg-surface-dim md:min-h-[28rem]">
            ...
            <div className={["absolute inset-0", isAnonymous ? "blur-[4px]" : ""].join(" ")}>
              ...
              <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black/80 to-transparent p-6">
                <h3 className="text-xl font-headline font-black uppercase tracking-tighter text-white">{garden.title}</h3>
                <p className="mt-2 text-xs font-body text-on-surface-variant">{garden.description}</p>
              </div>
            </div>
