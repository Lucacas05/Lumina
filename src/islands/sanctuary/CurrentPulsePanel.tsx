...
import { getCurrentProfileSummary, getCurrentRoom, useSanctuaryStore } from "@/lib/sanctuary/store";
...
function getPulseSubtitle(
  latestChronicle: ReturnType<typeof getCurrentProfileSummary>["latestChronicle"],
      ...
      <div className={["relative flex flex-col gap-8 p-8 md:flex-row md:items-center", isAnonymous ? "blur-[5px]" : ""].join(" ")}>
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <p className="gsap-rise mb-4 font-headline text-sm font-bold uppercase tracking-widest text-outline">
            {getPulseSubtitle(summary.latestChronicle, activeRoom?.name ?? null)}
          </p>

          <div className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-4">
            <div className="gsap-rise border-b-4 border-primary bg-surface-container-low p-4">
              <p className="text-[10px] font-headline font-bold uppercase tracking-widest text-outline">Sesiones</p>
              <p className="text-2xl font-headline font-black text-primary">{summary.sessionsCount}</p>
            </div>
            <div className="gsap-rise border-b-4 border-tertiary bg-surface-container-low p-4">
              <p className="text-[10px] font-headline font-bold uppercase tracking-widest text-outline">Horas de foco</p>
              <p className="text-2xl font-headline font-black text-tertiary">{summary.focusHours} h</p>
            </div>
            <div className="gsap-rise border-b-4 border-secondary bg-surface-container-low p-4">
              <p className="text-[10px] font-headline font-bold uppercase tracking-widest text-outline">Días activos</p>
              <p className="text-2xl font-headline font-black text-secondary">{summary.archiveDays}</p>
            </div>
            <div className="gsap-rise border-b-4 border-primary bg-surface-container-low p-4">
              <p className="text-[10px] font-headline font-bold uppercase tracking-widest text-outline">Hitos abiertos</p>
              <p className="text-2xl font-headline font-black text-primary">{summary.achievementsCount}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {isAnonymous && (
        <div className="absolute inset-0 flex items-center justify-center bg-surface/45 backdrop-blur-[2px]">
          <div className="mx-6 max-w-md border-2 border-outline-variant bg-surface-container px-6 py-5 text-center shadow-[0_10px_30px_rgba(0,0,0,0.3)]">
            ...
            <p className="font-headline text-[10px] font-bold uppercase tracking-[0.25em] text-outline">Pulso personal</p>
